...
void ShaderStorage::AddComputeShaderFile(const std::string &programKey, const std::string &filePath)
{
    if (_shaderBinaries.find(programKey) == _shaderBinaries.end())
    {
    ...
    _computeShaderContents += shaderData.str();
}

void ShaderStorage::CompileComputeShader(const std::string &programKey)
{
    if (_shaderBinaries.find(programKey) == _shaderBinaries.end())
    {
        fprintf(stderr, "Could not compile shader.  No program key '%s'.\n", programKey.c_str());
        return;
    }


    // OpenGL takes pointers to file contents and pointers to file content lengths, so use arrays
    const GLchar *bytes[] = { _computeShaderContents.c_str() };
    const GLint strLengths[] = { (int)_computeShaderContents.length() };
    ...
    if (isCompiled == GL_FALSE)
    {
        ...
        fprintf(stderr, "compute shader compilation of '%s' failed: '%s'\n", programKey.c_str(), errLog);
        glDeleteShader(shaderId);
    }
    else
    {
...
GLuint ShaderStorage::LinkComputeShader(const std::string &programKey)
{
    _BINARY_MAP::iterator itr = _shaderBinaries.find(programKey);
    if (itr == _shaderBinaries.end() ||
        itr->second.empty()) 
    {
        fprintf(stderr, "No binaries for program key '%s'.  Could not link shader.\n", programKey.c_str());
        return 0;
    }

    GLuint programId = glCreateProgram();

    GLuint computeShaderId = itr->second[0];
